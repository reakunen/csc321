...
# Alice computes shared secret and symmetric key
shared_secret_alice = pow(YB, XA, q)
k_alice = generate_symmetric_key(shared_secret_alice)

# Bob computes shared secret and symmetric key
...
plaintext = b"skibidy gyat ryiz rizzler alice!"
...
cipher = AES.new(k_bob, AES.MODE_CBC, iv)
plaintext = b"erm what the sigma bob?!!"
ciphertext = cipher.encrypt(pad(plaintext, AES.block_size))
